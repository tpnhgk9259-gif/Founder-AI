...
const SlideCover = ({ s = LUMEN }) => (
  <div className="pdf-sheet">
    ...
    <Eyebrow x={14} y={48} size={9} accent={PDF_COLORS.orange}>{s.stage}</Eyebrow>
    <Title x={14} y={58} size={82} color={PDF_COLORS.ink}>LUMEN</Title>
    <SerifAccent x={14} y={92} size={22} color={PDF_COLORS.muted} w={140}>{s.tagline}</SerifAccent>
...
const SlideProblem = ({ s = LUMEN, n, total }) => (
  <div className="pdf-sheet">
    ...
    <TitleBlock x={14} y={30} size={40} lines={[
      { text: "L'ÉNERGIE, 2e POSTE" },
      { text: 'DE CHARGES' },
      { text: '— ignoré.', serif: true, size: 28, color: PDF_COLORS.magenta, mt: 2 },
    ]}/>

    {/* Left stats */}
    <Abs x={14} y={72}>
      <div style={{ fontFamily: 'Anton, sans-serif', fontSize: 80, color: PDF_COLORS.magenta, lineHeight: 0.9 }}>18 %</div>
      <div style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 14, color: PDF_COLORS.muted, marginTop: 6, width: 58 * MM, lineHeight: 1.35 }}>
        du CA moyen en facture d'énergie pour un restaurant indépendant (2025).
      </div>
    </Abs>

    <Abs x={76} y={72}>
      <div style={{ fontFamily: 'Anton, sans-serif', fontSize: 80, color: PDF_COLORS.teal, lineHeight: 0.9 }}>28 h</div>
      <div style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 14, color: PDF_COLORS.muted, marginTop: 6, width: 54 * MM, lineHeight: 1.35 }}>
        par mois perdues à analyser les factures. Seul le comptable s'en occupe.
      </div>
    </Abs>
    ...
    <div style={{ position: 'absolute', right: 14 * MM, top: 72 * MM, width: 120 * MM, padding: 5 * MM, background: PDF_COLORS.paper2, borderLeft: `4px solid ${PDF_COLORS.ink}`, borderRadius: 4 }}>
      <div style={{ fontFamily: 'Geist Mono', fontSize: 8, letterSpacing: '0.16em', textTransform: 'uppercase', color: PDF_COLORS.muted, fontWeight: 500 }}>Entretien · chef Paris 11e</div>
      <div style={{ fontFamily: 'Georgia, serif', fontStyle: 'italic', fontSize: 15, marginTop: 8, lineHeight: 1.45, color: PDF_COLORS.ink }}>
        « Ma facture EDF double entre janvier et juillet. Je change de fournisseur tous les 2 ans au feeling. Personne n'est là pour me conseiller. »
      </div>
      <div style={{ fontFamily: 'Geist Mono', fontSize: 9, marginTop: 10, color: PDF_COLORS.ink, fontWeight: 600 }}>— Marc L., Bistrot des Batignolles</div>
    </div>
...
const SlideSolution = ({ s = LUMEN, n, total }) => (
  <div className="pdf-sheet">
    ...
    <TitleBlock x={14} y={30} size={40} lines={[
      { text: 'UN COPILOTE' },
      { text: 'ÉNERGIE' },
      { text: 'pour chaque resto.', serif: true, size: 28, color: PDF_COLORS.teal, mt: 2 },
    ]}/>

    <Body x={14} y={68} w={120} size={12} lh={1.55} color={PDF_COLORS.muted}>
      Lumen lit toutes les factures d'électricité, de gaz et de fioul d'un restaurant. Il détecte les anomalies, négocie automatiquement avec les fournisseurs et propose un plan de sobriété personnalisé — sans changer de matériel.
    </Body>

    {/* 3-step flow */}
    <div style={{ position: 'absolute', left: 14 * MM, top: 98 * MM, width: 252 * MM, display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 * MM }}>
      {[
        { n: '01', color: PDF_COLORS.orange, t: 'Connecter',  d: 'Upload PDF ou sync Enedis en 2 min. 1 seul compte.' },
        { n: '02', color: PDF_COLORS.teal,    t: 'Analyser',   d: 'Détection des 14 anomalies tarifaires courantes. Alertes en direct.' },
        { n: '03', color: PDF_COLORS.violet,  t: 'Agir',       d: 'Changement de contrat, plan de sobriété, attestation BDR.' },
      ].map(st => (
        <div key={st.n} style={{
...
const SlideMarket = ({ s = LUMEN, n, total }) => (
  <div className="pdf-sheet">
    ...
    <TitleBlock x={14} y={30} size={36} lines={[
      { text: 'UN PREMIER TAM DE 1,8 MD€ EN FRANCE.' },
    ]}/>
    ...
    <Abs x={14} y={46}>
      <div style={{ width: 252 * MM, display: 'flex', flexDirection: 'column', gap: 2.5 * MM }}>
        {[
          { k: 'TAM', name: 'Restauration française',      sub: '175 000 établissements, tous formats',            big: '1,8', unit: 'Md€', widthPct: 100, bg: '#EFEBE0', border: '#C9C2B0', ink: PDF_COLORS.ink,    eyebrow: PDF_COLORS.muted,  textColor: PDF_COLORS.ink },
          { k: 'SAM', name: 'Indépendants 5–30 ETP',       sub: '72 000 restos dans notre zone d\'efficacité',     big: '720', unit: 'M€',  widthPct: 60,  bg: 'rgba(122, 90, 242, 0.14)', border: PDF_COLORS.violet, ink: PDF_COLORS.violet, eyebrow: PDF_COLORS.violet, textColor: PDF_COLORS.violet },
          { k: 'SOM', name: 'Île-de-France + 4 métropoles · 3 ans', sub: '4 800 restos payants · breakeven à 1 500', big: '48',  unit: 'M€',  widthPct: 28,  bg: PDF_COLORS.orange,            border: PDF_COLORS.orange, ink: '#fff',            eyebrow: '#fff',            textColor: '#fff' },
        ].map((r) => (
  ...
  <div className="pdf-sheet">
    ...
    <Abs x={14} y={62}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3 * MM, width: 252 * MM }}>
        {[
          { t: 'Solo',   p: '39 €', bg: PDF_COLORS.card, ink: PDF_COLORS.ink, cap: 'Restaurant seul' },
          { t: 'Growth', p: '99 €', bg: PDF_COLORS.ink, ink: PDF_COLORS.paper, cap: '2 à 5 établissements', featured: true },
          { t: 'Groupe', p: '249 €', bg: PDF_COLORS.card, ink: PDF_COLORS.ink, cap: '6 à 30 · pack enseigne' },
        ].map(plan => (
...
const SlideContact = ({ s = LUMEN, n, total }) => (
  <div className="pdf-sheet">
    ...
    <Abs x={14} y={108}>
      <div style={{ fontFamily: 'Geist Mono', fontSize: 9, letterSpacing: '0.16em', color: PDF_COLORS.muted, fontWeight: 600, textTransform: 'uppercase' }}>CEO · Lumen</div>
      <div style={{ fontFamily: 'Anton', fontSize: 32, marginTop: 1 * MM, textTransform: 'uppercase', lineHeight: 1 }}>JULIETTE MOREAU</div>
